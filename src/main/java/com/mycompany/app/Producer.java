...
public final class Producer {
  ...
  public static void main(final String[] args) throws Exception {
    ...
    String newOrderReqXMLString = "<ord:newOrderRequest eventId=\"10\" eventTime=\"008-09-\"9T0\":49:45\" startTime=\"014-09-19T00:18:33\" finishTime=\"006-08-19T18:\"7:14+01:00\" failTime=\"009-05-16T13:4\":\"8\" eventType=\"RESERVATION_RESPONSE\" xmlns:ord=\"http://xml.thehutgroup.com/orderEvents\">\n  <!--Optional:-->\n  <ord:link rel=\"http://www.test.com/foedere/ferant\" href=\"http://www.corp.org/et/turbine\" mediaType=\"string\"/>\n  <ord:orderNumber>string</ord:orderNumber>\n  <ord:orderLink rel=\"http://www.any.org/speluncis/profundum\" href=\"http://www.my.com/flammas/ac\" mediaType=\"string\"/>\n</ord:newOrderRequest>";

    msg = sess.createTextMessage(newOrderReqXMLString);

    for(int i = 0; i < 1000; i++) { prod.send(msg); }
    conn.close();

  }


    private static void sendGraphMessages(MessageProducer prod, Session sess, List<Tuple<String, String>>  edges) {
	try { 
	for(int i = 0; i < 1000; i++) {
	
	    Message msg = sess.createTextMessage(genRandomMessage(edges));
	    prod.send(msg);

	    Thread.sleep(5000);
	}
	} catch(Exception e) { System.out.println("Failed to send a message"); e.printStackTrace(); }
